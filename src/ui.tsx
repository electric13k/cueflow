 ...
 */
/**
 * A dial, for the values an operator reaches for without looking.
 *
 * A fader tells you where a value sits along a line. A dial tells you at a glance and takes a whole
 * hand: it is one big target instead of a thin track, which is what you want for master level in a
 * dark wing with one hand already busy. Faders stay for anything you compare against its neighbours;
 * dials are for the two or three you reach for on their own.
 *
 * Built on a native range input rather than pointer maths, so it is a real slider to a screen
 * reader, arrow keys and Home/End work for free, and the value cannot drift from what assistive
 * technology reads. The input sits invisibly over the face and is dragged vertically; the ring is
 * drawn from the same value.
 *
 * `size` is a pixel diameter because that is the point of the control. Nothing smaller than 96 is
 * worth drawing as a dial: below that the ring stops being readable across a room and a fader says
 * the same thing in less space.
 */
export function Dial({
  label, value, onChange, onChangeEnd, minValue = 0, maxValue = 1, step = 0.01,
  getValue, size = 132, className, isDisabled, "aria-label": ariaLabel,
}: {
  label?: ReactNode; value: number; onChange: (v: number) => void; onChangeEnd?: (v: number) => void;
  minValue?: number; maxValue?: number; step?: number; getValue?: (v: number) => string;
  size?: number; className?: string; isDisabled?: boolean; "aria-label"?: string;
}) {
  const span = maxValue - minValue || 1;
  const ratio = Math.min(1, Math.max(0, (value - minValue) / span));
  // A 270 degree sweep with the gap at the bottom, the way a physical dial leaves room for its stop.
  const SWEEP = 270;
  const angle = -135 + ratio * SWEEP;
  const r = size / 2 - 10;
  const circumference = 2 * Math.PI * r;
  const reading = getValue ? getValue(value) : String(value);

  return (
    <div className={cn("dial flex flex-col items-center gap-1.5", isDisabled && "opacity-50", className)}>
      <div className="relative" style={{ width: size, height: size }}>
        <svg viewBox={`0 0 ${size} ${size}`} width={size} height={size} aria-hidden className="-rotate-[135deg]">
          <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={9} strokeLinecap="round"
            className="dial__track" strokeDasharray={`${circumference * (SWEEP / 360)} ${circumference}`} />
          <circle cx={size / 2} cy={size / 2} r={r} fill="none" strokeWidth={9} strokeLinecap="round"
            className="dial__fill" strokeDasharray={`${circumference * (SWEEP / 360) * ratio} ${circumference}`} />
        </svg>
        <div className="dial__face pointer-events-none absolute inset-[14px] grid place-items-center rounded-full">
          <span className="font-mono text-heading font-semibold tabular-nums">{reading}</span>
        </div>
        {/* The pointer, drawn from the same ratio as the ring so they cannot disagree. */}
        <div className="pointer-events-none absolute inset-0 grid place-items-center">
          <div className="dial__pointer" style={{ transform: `rotate(${angle}deg)`, height: size - 26 }} />
        </div>
        {/*
          A vertical range input laid over the face. Dragging up raises the value, which is how a
          knob behaves under a finger; a rotary gesture is worse on a trackpad and impossible with a
          keyboard. `writing-mode` is how a range input is made vertical now that `appearance:
          slider-vertical` is gone.
        */}
        <input
          type="range" className="dial__input absolute inset-0 h-full w-full cursor-ns-resize opacity-0"
          style={{ writingMode: "vertical-lr", direction: "rtl" }}
          min={minValue} max={maxValue} step={step} value={value} disabled={isDisabled}
