...
// Preloaded sounds live in the public Supabase 'audio/preloaded' bucket when built with cloud vars; fall back to /audio locally.
const base = import.meta.env.VITE_SUPABASE_URL;
const assetUrl = (file: string) => base ? `${base}/storage/v1/object/public/audio/preloaded/${encodeURIComponent(file)}` : `/audio/${encodeURIComponent(file)}`;
export const initialTracks = (): Track[] => [...new Map(cues.map(([, file]) => [file, { id: file, title: file.replace(/\.mp3$/, "").replace(/[-_]/g, " "), url: assetUrl(file), effects: { ...effects }, createdAt: new Date().toISOString() }])).values()];
