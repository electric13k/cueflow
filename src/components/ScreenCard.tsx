    ...
    <motion.div {...fade(delay)} className={mobileOnly ? "tutorial-phone-only" : undefined} style={{ perspective: "1400px" }}>
      ...
      >
        <span className="glass glass-hover absolute inset-0 flex flex-col overflow-hidden p-2" style={{ backfaceVisibility: "hidden" }}>
          ...
          <span className="flex items-center justify-between px-2 py-2 text-body font-bold">
            {title}
            <span className="text-label font-medium text-muted">Flip →</span>
          </span>
        </span>
        <span
        ...
        >
          ...
          <span className="mt-2 text-label text-muted">← Flip back</span>
        </span>
      </button>
    </motion.div>
  );
