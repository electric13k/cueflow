  ...
  return (
    <div className="glass-soft mt-4 p-5">
      <ol className="space-y-4">
        <li>
          <p className="text-body font-semibold"><span className="text-accent">1.</span> What's this about?</p>
          <div className="mt-2 flex flex-wrap gap-2">
            {topics.map(t => (
              <button key={t} onClick={() => setTopic(t)}
                className={`rounded-full border px-3 py-1.5 text-label font-semibold transition-colors ${topic === t ? "border-accent bg-accent/15 text-accent" : "border-white/10 text-muted hover:border-white/25"}`}>
                {t}
              </button>
            ))}
          </div>
        ...
        <li>
          <p className="text-body font-semibold"><span className="text-accent">2.</span> Tell us what happened</p>
          <textarea
            value={form.message} onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
            rows={4} maxLength={4000} placeholder="As much or as little detail as you like."
            className="mt-2 w-full rounded-2xl border border-white/10 bg-white/[.03] p-3 text-body outline-none placeholder:text-muted focus:border-accent/50"
          />
        </li>
        <li>
          <p className="text-body font-semibold"><span className="text-accent">3.</span> Where should we reply? {!account && <span className="font-normal text-muted">(optional)</span>}</p>
          {account ? (
            <p className="mt-2 flex items-center gap-2 rounded-2xl border border-white/10 bg-white/[.03] px-3 py-2.5 text-body">
      ...
      {error && <p className="mt-3 text-label text-warning">{error}</p>}
