...
export default function KeybindRow({ label, value, clash, onSet }: {
  ...
  return (
    <div className={`flex items-center justify-between gap-3 rounded-xl border px-3 py-2 ${clash ? "border-live/50 bg-live/10" : "border-white/10 bg-white/[.03]"}`}>
      <span className="text-body">{label}{clash && <span className="ml-2 text-label text-live">shared with another action</span>}</span>
      <button
        className={`min-w-24 rounded-lg border px-3 py-1.5 font-mono text-body ${listening ? "border-accent bg-accent/15 text-accent" : "border-white/15 bg-white/5"}`}
